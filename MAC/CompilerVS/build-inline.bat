...
msbuild LTE-MAC-inline.vcxproj /target:rebuild /p:Configuration=Release /p:Platform=x64 /m /verbosity:minimal %MODE%

if %errorlevel% NEQ 0 (
  del LTE-MAC-inline\x64\Release\LTE-inline.exe 
  set errorlev=1
) else (
set errorlev=0
)
